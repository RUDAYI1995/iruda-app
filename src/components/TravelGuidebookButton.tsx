...
export function TravelGuidebookButton() {
  ...
  return (
    <div className="relative shrink-0">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-1 rounded-full border border-amber-300 bg-amber-50 px-2.5 py-1.5 text-xs font-medium text-amber-800 shadow-sm transition-transform hover:scale-105 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-200"
        aria-expanded={open}
      >
